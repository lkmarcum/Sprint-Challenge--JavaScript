...
graduates.forEach(function(object) {
  universities.push(object.university);
});
universities.sort();
...
let uni = [];
uni = graduates.filter(object => object.university.includes("Uni"));
console.log(uni);
...
let lowerCase = [];
lowerCase = zooAnimals.map(object => object.animal_name.toLowerCase());
...
*/
let lowerPopulation = [];
lowerPopulation = zooAnimals.filter(object => object.population < 5);
...
populationTotal = zooAnimals.reduce(function(acc, object) {
  acc = acc + object.population;
  return acc;
}, 0);
